...
async def on_chat_start():
    ...
    KBase_auth_token = await cl.AskUserMessage(
        "Please enter your KBase token key here:"
    ).send()
    ...
    task1_descr = (
        user_query
        + """ Please suggest only one app at a time for a particular step. If you must suggest alternative apps please limit to only one."""
    )
