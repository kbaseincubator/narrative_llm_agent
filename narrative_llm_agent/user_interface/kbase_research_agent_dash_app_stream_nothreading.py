...
def create_approval_display():
    approval_req = app_state.get('approval_request', {})
    analysis_plan = approval_req.get('analysis_plan', [])
    
    return [
        html.Div([
            html.H3("🔍 Workflow Paused - Human Approval Required", 
                   style={'color': COLORS['warning'], 'textAlign': 'center'}),
            
            html.Div([
                html.H4("📋 Analysis Plan Review"),
                html.P(f"Narrative ID: {approval_req.get('narrative_id')}"),
                html.P(f"Reads ID: {approval_req.get('reads_id')}"),
                html.P(f"Description: {approval_req.get('description', '')[:100]}..."),
                
                html.H5("Proposed Analysis Steps:"),
                html.Div([
                    html.Div([
                        html.H6(f"Step {i+1}: {step.get('Name', 'Unnamed Step')}", 
                               style={'color': COLORS['primary']}),
                        html.P(f"App: {step.get('App', 'Unknown App')}"),
                        html.P(f"App ID: {step.get('app_id', 'Unknown ID')}"),
                        html.P(f"Description: {step.get('Description', 'No description')}"),
                        html.P(f"Creates new object: {'Yes' if step.get('expect_new_object', False) else 'No'}")
                    ], style={'border': '1px solid #ddd', 'padding': '10px', 'margin': '5px', 'borderRadius': '4px'})
                ...
                html.Div([
                    ...
                    html.Button("❌ Request Revisions", id='reject-btn', style=WARNING_BUTTON),
                    html.Button("🛑 Cancel Workflow", id='cancel-btn', style=DANGER_BUTTON)
                ], style={'textAlign': 'center', 'marginTop': '20px'}),
                
                html.Div(id='approval-feedback-area')
            ])
        ], style={**CARD_STYLE, 'border': f'2px solid {COLORS["warning"]}'})
    ]
...
def handle_approval_decision(approve_clicks, reject_clicks, cancel_clicks, submit_clicks, feedback_text):
    ctx = callback_context
    if not ctx.triggered:
        return ""
    ...
    elif trigger_id == 'submit-feedback-btn' and submit_clicks:
        resume_input = {
            "rejected": True,
            "feedback": feedback_text or "Please revise the analysis plan."
        }
        message = "📝 Feedback submitted. Regenerating analysis plan..."
    ...
    if resume_input:
        ...
        if current_workflow and 'workflow' in current_workflow and 'thread_id' in current_workflow:
            try:
                ...
                return html.Div(error_msg, style={'color': COLORS['error']})
...
def run_genome_analysis_with_hitl(narrative_id, reads_id, description, credentials):
    ...
    try:
        ...
        # Start the workflow directly - NO THREADING
        print("DEBUG: Starting workflow execution directly (no threading)")
