...
def create_credentials_form():
    return dbc.Card([
        dbc.CardHeader("🔑 KBase Credentials"),
        dbc.CardBody([
            dbc.Row([
                dbc.Col([
            ...
            dbc.Button("Save Credentials", id="save-credentials-btn", color="primary")
        ])
    ])
        ...
        dbc.CardBody([
            dbc.Row([
                dbc.Col([
                    ...
                    dbc.Input(id="narrative-id", value="217789", type="text")
                ], width=6),
                dbc.Col([
                    dbc.Label("Reads ID"),
                    dbc.Input(id="reads-id", value="217789/2/1", type="text")
                ], width=6)
            ], className="mb-3"),
            ...
            dbc.Row([
                ...
                dbc.Col([
                    dbc.Label("Organism"),
                    dbc.Input(id="organism", value="Bacillus subtilis sp. strain UAMC", type="text")
                ], width=4),
                dbc.Col([
...
The goal is to have a complete annotated genome and classify the microbe."""
            ...
            dbc.Button("🚀 Generate Analysis Plan", id="run-analysis-btn", color="success", size="lg")
        ])
    ])
...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col([
            ...
            html.P("Automated genome analysis workflows with human approval", className="lead")
        ])
    ], className="mb-4"),
...
def create_approval_interface(workflow_state):
    ...
    return dbc.Card([
        ...
        dbc.CardBody([
            ...
            dbc.Row([
                dbc.Col([
                    dbc.Button("✅ Approve Plan", id="approve-btn", color="success", size="lg", className="me-2"),
                    dbc.Button("❌ Reject Plan", id="reject-btn", color="danger", size="lg", className="me-2"),
                    dbc.Button("🚫 Cancel", id="cancel-btn", color="secondary", size="lg")
                ], width=12)
            ], className="mb-3"),
            
            # Feedback area (initially hidden)
            html.Div(id="feedback-area", style={"display": "none"}, children=[
                html.Hr(),
                ...
                dbc.Button("Submit Feedback", id="submit-feedback-btn", color="warning", className="mt-2")
            ]),
        ...
        if final_state.get("results"):
            components.append(
                dbc.Card([
                    ...
                    dbc.CardBody([
                        html.Pre(str(final_state["results"]))
                    ])
                ], className="mb-3")
            )
...
@app.callback(
    [Output("workflow-state-store", "data"),
     Output("analysis-results", "children")],  # Remove the analysis-loading output
...
def run_analysis_planning_callback(n_clicks, credentials, narrative_id, reads_id, description):
    ...
    if button_id == "approve-btn" and approve_clicks:
        ...
        try:
            ...
            if execution_result.get("status") == "completed":
                ...
                error_status = dbc.Alert(f"❌ Execution failed: {execution_result.get('error', 'Unknown error')}", color="danger")
...
def generate_mra(n_clicks, credentials, narrative_id):
    ...
    if n_clicks and credentials and credentials.get("kb_auth_token"):
        ...
        loading_alert = dbc.Alert("🔄 Generating MRA draft... This may take a few minutes.", color="info")
