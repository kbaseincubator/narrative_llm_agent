...
def test_list_objects_tool(mock_llm, mocker):
    ...
    mock = mocker.patch.object(
        Workspace, "list_workspace_objects", return_value=obj_list
    )
...
def test_get_report_from_job_id_no_report(
    status, job_output, expected, mock_job_states, mocker, mock_llm
):
    job_id = "job_id_1"
    ...
    mock = mocker.patch.object(
        ExecutionEngine, "check_job", return_value=JobState(state)
    )
    wa = WorkspaceAgent(mock_llm)
